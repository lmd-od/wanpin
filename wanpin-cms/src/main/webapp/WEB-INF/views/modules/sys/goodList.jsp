...
<body>
	...
	<form:form id="searchForm" modelAttribute="good" action="${ctx}/sys/good/list" method="post" class="breadcrumb form-search ">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr>
		         ...
		         <th>创建时间</th>
		         <th>修改时间</th>
		         <shiro:hasPermission name="sys:good:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="good">
			<tr>
				<td>${good.goodsId }</td>
				<td>${good.goodsSource }</td>
				<td>${good.goodsStatus }</td>
				<td>${good.goodsName }</td>
				<td>${good.goodsMoney }</td>
				<td>${good.goodsCover }</td>
				...
				<td><fmt:formatDate value="${good.updateTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
